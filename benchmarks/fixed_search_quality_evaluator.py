#!/usr/bin/env python3
"""
Fixed Search Quality Evaluation for AST vs Naive Chunking

This version fixes the text extraction issue that was causing all search quality 
...
class FixedSearchQualityEvaluator:
    """Fixed search quality evaluator that properly extracts chunk text."""
    ...
    def extract_text_from_chunk(self, chunk):
        """Extract actual text content from chunk, handling different formats."""
    ...
    def evaluate_search_quality(self) -> Dict[str, Any]:
        ...
        logger.info("Starting fixed search quality evaluation...")
    ...
    def _generate_chunks(self, use_ast: bool) -> List[Dict[str, Any]]:
        ...
        # Process and fix chunks
        enriched_chunks = []
        for i, raw_chunk in enumerate(raw_chunks):
            # Extract actual text content
            chunk_text = self.extract_text_from_chunk(raw_chunk)
            ...
            enriched_chunk = {
                ...
                "source_file": self._guess_source_file(chunk_text),
                "contains_functions": self._contains_function_definitions(chunk_text),
                "contains_classes": self._contains_class_definitions(chunk_text),
                "contains_imports": self._contains_imports(chunk_text),
                "is_complete_construct": self._is_complete_code_construct(chunk_text)
            }
            enriched_chunks.append(enriched_chunk)
            
        logger.info(f"{approach} chunking: {len(enriched_chunks)} valid chunks from {len(raw_chunks)} raw chunks in {processing_time:.2f}s")
    ...
    def _evaluate_single_query(self, chunks: List[Dict], query: Dict, approach: str) -> Dict[str, Any]:
        ...
        return {
            ...
            "approach": approach,
            "metrics": metrics,
            "relevant_chunks": relevant_chunks[:5]  # Store top 5 for analysis
        }
    ...
    # Helper methods (same as original but working on proper text)
    ...
    def _is_complete_code_construct(self, chunk: str) -> bool:
        ...
        # For Python, check basic completeness
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Fixed search quality evaluation")
    parser.add_argument(
        "--test-data-dir",
        type=str,
    ...
    try:
        evaluator = FixedSearchQualityEvaluator(args.test_data_dir, args.queries_file)
        results = evaluator.evaluate_search_quality()
        
        # Print summary
        print("\n" + "="*60)
        print("FIXED SEARCH QUALITY EVALUATION SUMMARY")
        print("="*60)
        ...
        print(f"\nRelevance Scores:")
        print(f"  AST Avg Score:    {ast_results.get('avg_relevance_score', 0):.2f}")
        print(f"  Naive Avg Score:  {naive_results.get('avg_relevance_score', 0):.2f}")
        print(f"  Improvement:      {comparison.get('avg_relevance_improvement', 0):+.2f}")
        
        # Save results
        output_file = "fixed_search_quality_results.json"
